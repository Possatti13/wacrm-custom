...
export class XAiStructuredExtractor implements CommercialIntelligenceProvider {
  readonly providerName = 'xai'

  constructor(private readonly apiKey: string) {
    if (!apiKey) throw new Error('XAiStructuredExtractor requires an API key')
  }
  ...
  async extract(request: ExtractionProviderRequest): Promise<ExtractionProviderResult> {
    ...
    try {
      ...
      if (!res.ok) {
        throw await providerHttpError('xAI', res)
      }
